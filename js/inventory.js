...
class InventorySystem {
    constructor(game) {
        ...
        this.itemTemplates = {
            ...
            laboratory: {
                ...
                cooldown: -1, // 被动效果，无冷却
                minQuality: 3, // 紫色起始
                description: '每有一个魔法物品，给所有物品+8攻击力'
            },
            crystal: {
                name: '魔力水晶',
                icon: '💎',
    ...
    buyItem(shopIndex) {
        ...
        if (existingItemIndex !== -1) {
            ...
            if (hasSpace) {
                ...
                // 为炼金工房计算品质升级效果（相对于起始品质）
                if (item.id === 'alchemyLab') {
                    const qualityLevelsAboveMin = item.quality - template.minQuality;
                    // 蓝1+2，紫2+3，橙3+4，红4+5
                    item.alchemyGoldGeneration = 1 + qualityLevelsAboveMin;
                    item.alchemySellBonus = 2 + qualityLevelsAboveMin;
                }
    ...
    produceUnits(item, itemSlot) {
        ...
        // 炼金工房特殊处理：产生金币
        if (item.id === 'alchemyLab') {
            this.triggerAlchemyLabGoldGeneration(item);
            return;
        }
        
        // 魔力水晶特殊处理：主动充能
        if (item.id === 'crystal') {
            this.triggerCrystalActiveCharge(item);
            return;
        }
    ...
    sellItem(slotIndex) {
        ...
        // 计算炼金工房出售加成
        const alchemyBonus = this.getAlchemySellBonus();
        const totalGold = sellPrice + alchemyBonus;
        
        // 获得金币
        this.game.playerGold += totalGold;
        
        if (alchemyBonus > 0) {
            console.log(`炼金工房出售加成：+${alchemyBonus}金币`);
        }
        ...
        // 刷新学徒效果（因为可能出售了学徒或魔法物品）
        this.applyApprenticeBonus();
        
        // 刷新法杖效果（因为可能出售了法杖或魔法物品）
        this.applyStaffBonus();
        this.applyLaboratoryBonus();
    ...
    moveItemFromBackpackToInventory(fromBackpackSlot, toInventorySlot) {
        ...
        // 刷新学徒效果（物品位置变化可能影响学徒效果）
        this.applyApprenticeBonus();
        
        // 刷新法杖效果（物品位置变化可能影响法杖效果）
        this.applyStaffBonus();
        this.applyLaboratoryBonus();
    ...
    buyItemToBackpack(shopIndex, targetSlot) {
        ...
        if (canPlaceAtSlot) {
            ...
            // 为炼金工房计算品质升级效果（相对于起始品质）
            if (newItem.id === 'alchemyLab') {
                const qualityLevelsAboveMin = newItem.quality - template.minQuality;
                // 蓝1+2，紫2+3，橙3+4，红4+5
                newItem.alchemyGoldGeneration = 1 + qualityLevelsAboveMin;
    ...
    showItemInfo(item, slotElement) {
        ...
        if (item.id === 'assassin' && specialSkillElement) {
            ...
            const skillStatusElement = document.getElementById('skill-status');
            skillStatusElement.style.display = 'none'; // 隐藏状态信息
        } else if (item.id === 'apprentice' && specialSkillElement) {
            specialSkillElement.classList.remove('hidden');
            specialSkillElement.classList.add('passive'); // 被动效果，添加分割线
            
            const skillDescElement = document.getElementById('skill-description');
            if (skillDescElement) {
                const qualityBonus = this.getApprenticeQualityBonus(item.quality);
                skillDescElement.innerHTML = `每有一个<span class="mage-badge">魔法</span>单位，攻击力 <span style="color: white; font-weight: bold;">+${qualityBonus}</span>`;
            }
        ...
        } else if (item.id === 'staff' && specialSkillElement) {
            specialSkillElement.classList.remove('hidden');
            specialSkillElement.classList.add('passive'); // 被动效果，添加分割线
        ...
        } else if (item.id === 'golem' && specialSkillElement) {
            specialSkillElement.classList.remove('hidden');
            specialSkillElement.classList.add('passive'); // 被动效果，添加分割线
            
            const skillDescElement = document.getElementById('skill-description');
            if (skillDescElement) {
                const qualityBonus = this.getGolemAdjacentBonus(item.quality);
                skillDescElement.innerHTML = `如果有相邻的<span class="mage-badge">魔法</span>单位，攻击力 <span style="color: white; font-weight: bold;">+${qualityBonus}</span>`;
            }
        ...
        } else if (item.id === 'alchemist' && specialSkillElement) {
            specialSkillElement.classList.remove('hidden');
            specialSkillElement.classList.remove('passive'); // 主动效果，无分割线
            
            const skillDescElement = document.getElementById('skill-description');
            if (skillDescElement) {
                skillDescElement.innerHTML = `为相邻<span class="mage-badge">魔法</span>物品 <span style="color: #17a2b8; font-weight: bold;">加速1秒</span>`;
            }
        ...
        } else if (item.id === 'witch' && specialSkillElement) {
            specialSkillElement.classList.remove('hidden');
            specialSkillElement.classList.add('passive'); // 被动效果，添加分割线
            
            const skillDescElement = document.getElementById('skill-description');
            if (skillDescElement) {
                const targetCount = this.getWitchTargetCount(item.quality);
                skillDescElement.innerHTML = `同时攻击 <span style="color: white; font-weight: bold;">${targetCount}个</span> 敌人`;
            }
        ...
        } else if (item.id === 'laboratory' && specialSkillElement) {
            specialSkillElement.classList.remove('hidden');
            specialSkillElement.classList.add('passive'); // 被动效果，添加分割线
            
            const skillDescElement = document.getElementById('skill-description');
            if (skillDescElement) {
                const bonusPerMage = this.getLaboratoryQualityBonus(item.quality);
                skillDescElement.innerHTML = `每有一个<span style="color: #9b59b6; font-weight: bold;">魔法物品</span>，给所有物品 <span style="color: white; font-weight: bold;">+${bonusPerMage}攻击力</span>`;
            }
        ...
        } else if (item.id === 'alchemyLab' && specialSkillElement) {
            specialSkillElement.classList.remove('hidden');
            specialSkillElement.classList.remove('passive'); // 主动效果，无分割线
            
            const skillDescElement = document.getElementById('skill-description');
            if (skillDescElement) {
                const goldAmount = item.alchemyGoldGeneration || 1;
                const sellBonus = item.alchemySellBonus || 2;
                skillDescElement.innerHTML = `获得 <span style="color: #f1c40f; font-weight: bold;">${goldAmount}金币</span><br>` +
                                           `<div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid rgba(255,255,255,0.2);">即使在背包也生效，出售物品额外<span style="color: #f1c40f; font-weight: bold;">+${sellBonus}金币</span></div>`;
            }
        ...
        } else if (item.id === 'crystal' && specialSkillElement) {
            specialSkillElement.classList.remove('hidden');
            specialSkillElement.classList.remove('passive'); // 主动效果，无分割线
            
            const skillDescElement = document.getElementById('skill-description');
            if (skillDescElement) {
                const chargeAmount = this.getCrystalChargeAmount(item.quality);
                skillDescElement.innerHTML = `为另一个小型物品 <span style="color: #17a2b8; font-weight: bold;">充能${chargeAmount}秒</span><br>` +
                                           `<div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid rgba(255,255,255,0.2);">被加速时为自身 <span style="color: #17a2b8; font-weight: bold;">充能${chargeAmount}秒</span></div>`;
            }
    ...
    getUnitStats(unitType, unitKey = null, item = null) {
        ...
        if (unitKey) {
            ...
            if (specificStats[unitKey]) {
                ...
                // 法杖技能：添加魔法物品攻击力加成（适用于所有魔法物品）
                if (item && item.staffBonus && item.template.unitType === 'mage') {
                    stats.attack += item.staffBonus;
                }
                ...
                // 实验室全局攻击力加成
                if (item && item.laboratoryBonus) {
                    stats.attack += item.laboratoryBonus;
                }
    ...
    applySingleBarbarianEffect(barbarianSlot) {
        ...
        // 使用通用相邻查找函数，过滤近战单位
        const adjacentMeleeItems = this.findAdjacentItems(
            barbarianSlot, 
            barbarianItem.template.size, // 考虑野蛮人的尺寸
            (item) => item.template.unitType === 'melee'
        );
        
        for (const adjacent of adjacentMeleeItems) {
            const { item: targetItem, position } = adjacent;
            
            targetItem.barbarianAdjacentBonus = (targetItem.barbarianAdjacentBonus || 0) + bonus;
            console.log(`野蛮人为${position}侧近战物品(${targetItem.id})提供攻击力加成 +${bonus}`);
        }
    }
        ...
        for (let i = 0; i < this.inventory.length; i++) {
            ...
            if (golemItem && golemItem !== 'occupied' && golemItem.id === 'golem') {
                ...
                if (hasAdjacentMage) {
                    ...
                    console.log(`魔偶(位置${i})检测到相邻魔法物品，获得攻击力加成 +${bonus}`);
                } else {
                    console.log(`魔偶(位置${i})没有相邻的魔法物品，无加成`);
                }
            }
        }
    ...
    hasAdjacentMageUnit(slotIndex) {
        ...
        if (adjacentMageItems.length > 0) {
            for (const adjacent of adjacentMageItems) {
                console.log(`魔偶(位置${slotIndex})${adjacent.position}侧发现魔法物品: ${adjacent.item.id}`);
            }
            return true;
        } else {
            console.log(`魔偶(位置${slotIndex})未发现相邻魔法物品`);
            return false;
        }
    }
    
    // 查找物品在inventory中的位置索引
    findItemSlot(targetItem) {
        for (let i = 0; i < this.inventory.length; i++) {
            const item = this.inventory[i];
            if (item === targetItem) {
                return i;
    ...
    applyApprenticeBonus() {
        ...
        for (let i = 0; i < this.inventory.length; i++) {
            ...
            if (apprenticeItem && apprenticeItem !== 'occupied' && apprenticeItem.id === 'apprentice') {
                // 计算当前战斗区中的魔法物品数量（包括召唤和非召唤的魔法物品）
                let mageUnitCount = 0;
                for (let j = 0; j < this.inventory.length; j++) {
                    const item = this.inventory[j];
                    if (item && item !== 'occupied' && 
                        item.template.unitType === 'mage') {
                        mageUnitCount++;
                    }
                }
                
                // 计算并应用加成
                const bonusPerUnit = this.getApprenticeQualityBonus(apprenticeItem.quality);
                const totalBonus = mageUnitCount * bonusPerUnit;
                apprenticeItem.apprenticeBonus = totalBonus;
                console.log(`学徒检测到${mageUnitCount}个魔法物品，获得攻击力加成 +${totalBonus} (${bonusPerUnit}×${mageUnitCount})`);
            }
        }
    ...
    applyStaffBonus() {
        ...
        for (let i = 0; i < this.inventory.length; i++) {
            ...
            if (staffItem && staffItem !== 'occupied' && staffItem.id === 'staff') {
                ...
                // 为所有魔法物品添加法杖攻击力加成
                for (let j = 0; j < this.inventory.length; j++) {
                    const mageItem = this.inventory[j];
                    if (mageItem && mageItem !== 'occupied' && 
                        mageItem.template.unitType === 'mage') {
                        
                        // 初始化staffBonus属性
                        if (!mageItem.staffBonus) {
                            mageItem.staffBonus = 0;
                        }
                        
                        // 累加法杖加成（支持多个法杖）
                        mageItem.staffBonus += totalStaffBonus;
                    }
                }
                
                console.log(`法杖提供攻击力加成: 基础+${baseBonus} + 获胜累积+${winBonus} = 总共+${totalStaffBonus}`);
            }
        }
    ...
    // 应用实验室的全局攻击力加成效果
    applyLaboratoryBonus() {
        // 清除所有物品的实验室攻击力加成
        for (let i = 0; i < this.inventory.length; i++) {
            const item = this.inventory[i];
            if (item && item !== 'occupied') {
                // 移除实验室加成（保留其他加成）
                if (item.laboratoryBonus) {
                    item.laboratoryBonus = 0;
                }
            }
        }
        ...
        // 重新计算实验室加成
        for (let i = 0; i < this.inventory.length; i++) {
            const labItem = this.inventory[i];
            if (labItem && labItem !== 'occupied' && labItem.id === 'laboratory') {
                // 计算当前战斗区中的魔法物品数量
                let mageItemCount = 0;
                for (let j = 0; j < this.inventory.length; j++) {
                    const item = this.inventory[j];
                ...
                // 根据实验室品质计算攻击力加成
                const bonusPerMage = this.getLaboratoryQualityBonus(labItem.quality);
                const totalLaboratoryBonus = mageItemCount * bonusPerMage;
                
                console.log(`实验室检测到${mageItemCount}个魔法物品，总攻击力加成: ${mageItemCount}×${bonusPerMage}=${totalLaboratoryBonus}`);
                
                // 为所有物品添加实验室攻击力加成（不限类型）
                for (let j = 0; j < this.inventory.length; j++) {
                    const targetItem = this.inventory[j];
                    if (targetItem && targetItem !== 'occupied') {
                        // 初始化laboratoryBonus属性
                        if (!targetItem.laboratoryBonus) {
                            targetItem.laboratoryBonus = 0;
                        }
                        
                        // 累加实验室加成（支持多个实验室）
                        targetItem.laboratoryBonus += totalLaboratoryBonus;
                    }
                }
                
                console.log(`实验室为所有物品提供攻击力加成: +${totalLaboratoryBonus}`);
            }
        }
    ...
    addAccelerationTime(item, accelerationTime) {
        ...
        // 魔力水晶特殊效果：被加速时充能自己
        if (item.id === 'crystal' && accelerationTime > 0) {
            console.log(`魔力水晶被加速${accelerationTime/60}秒，触发自我充能`);
            this.triggerCrystalSelfCharge(item);
        }
    ...
    // 魔力水晶主动充能：为另一个小型物品充能
    triggerCrystalActiveCharge(crystalItem) {
        const chargeAmount = this.getCrystalChargeAmount(crystalItem.quality);
        const chargeFrames = chargeAmount * 60; // 转换为帧数
        
        console.log(`魔力水晶(品质${crystalItem.quality})开始主动充能，充能时长: ${chargeAmount}秒`);
        
        // 找到所有小型物品（size=1）作为充能目标
        const smallItems = [];
        for (let i = 0; i < this.inventory.length; i++) {
            const item = this.inventory[i];
            if (item && item !== 'occupied' && 
                item.template.size === 1 && 
                item.id !== 'crystal' && // 不充能其他魔力水晶
                item.cooldownRemaining > 0) { // 只充能在冷却中的物品
                smallItems.push({item, index: i});
            }
        }
        
        if (smallItems.length === 0) {
            console.log('魔力水晶主动充能：没有找到可充能的小型物品');
            return;
        }
        
        // 随机选择一个小型物品进行充能
        const randomIndex = Math.floor(Math.random() * smallItems.length);
        const {item: targetItem, index: targetIndex} = smallItems[randomIndex];
        
        // 执行充能
        const oldCooldown = targetItem.cooldownRemaining;
        targetItem.cooldownRemaining = Math.max(0, targetItem.cooldownRemaining - chargeFrames);
        
        // 如果充能后冷却时间归零，标记为准备就绪
        if (targetItem.cooldownRemaining === 0) {
            targetItem.isReady = true;
        }
        
        console.log(`✓ 魔力水晶为${targetItem.template.name}(位置${targetIndex})充能${chargeAmount}秒，冷却时间: ${Math.round(oldCooldown/60*10)/10}s -> ${Math.round(targetItem.cooldownRemaining/60*10)/10}s`);
    ...
    // 魔力水晶被动响应：被加速时充能自己
    triggerCrystalSelfCharge(crystalItem) {
        const chargeAmount = this.getCrystalChargeAmount(crystalItem.quality);
        const chargeFrames = chargeAmount * 60; // 转换为帧数
        
        // 只有在冷却中才能充能自己
        if (crystalItem.cooldownRemaining > 0) {
            const oldCooldown = crystalItem.cooldownRemaining;
            crystalItem.cooldownRemaining = Math.max(0, crystalItem.cooldownRemaining - chargeFrames);
            
            // 如果充能后冷却时间归零，标记为准备就绪
            if (crystalItem.cooldownRemaining === 0) {
                crystalItem.isReady = true;
            }
            
            console.log(`✓ 魔力水晶自我充能${chargeAmount}秒，冷却时间: ${Math.round(oldCooldown/60*10)/10}s -> ${Math.round(crystalItem.cooldownRemaining/60*10)/10}s`);
            
            // 更新显示
            this.updateInventoryDisplay();
        } else {
            console.log('魔力水晶自我充能：当前不在冷却中，无需充能');
        }
    }
    
    // 获取魔力水晶品质对应的充能时间
    getCrystalChargeAmount(quality) {
        const chargeMap = {
            3: 1, // 紫色：1秒
            4: 2, // 橙色：2秒
            5: 3  // 红色：3秒
    ...
    onBattleVictory() {
        for (let i = 0; i < this.inventory.length; i++) {
            const item = this.inventory[i];
            if (item && item !== 'occupied' && item.id === 'staff') {
                // 根据法杖品质获取每次获胜的加成
                const winBonus = this.getStaffWinBonus(item.quality);
                
                // 增加法杖的获胜累积加成
                item.staffWinBonus = (item.staffWinBonus || 0) + winBonus;
                
                console.log(`法杖成长！品质${item.quality}，本次+${winBonus}，累积获胜加成：${item.staffWinBonus}`);
            }
        }
    ...
    // 获取女巫品质对应的攻击目标数量
    getWitchTargetCount(quality) {
        const targetMap = {
            3: 2, // 紫色：2个目标
            4: 3, // 橙色：3个目标  
            5: 4  // 红色：4个目标
        };
        return targetMap[quality] || 2;
    }
    ...
    triggerMagicSwordEffect(swordSlot) {
        ...
        const adjacentMeleeItems = this.findAdjacentItems(
            swordSlot, 
            swordItem.template.size, // 宝剑是小尺寸物品
            (item) => item.template.unitType === 'melee'
        );
        ...
        for (const adjacent of adjacentMeleeItems) {
            ...
            console.log(`宝剑触发！为${position}侧近战物品(${targetItem.id})增加攻击力 +${bonus}！当前总加成: +${targetItem.attackBonus}`);
        }
        
        if (adjacentMeleeItems.length > 0) {
            console.log(`宝剑效果触发完成，影响了 ${adjacentMeleeItems.length} 个相邻近战物品`);
            // 更新显示以反映攻击力变化
            this.updateInventoryDisplay();
        } else {
    ...
    // 通用的相邻物品查找函数
    findAdjacentItems(sourceSlot, sourceSize = 1, filterFn = null) {
        const adjacentItems = [];
        
        // 检查左侧相邻位置
        const leftSlot = sourceSlot - 1;
        if (leftSlot >= 0) {
            const leftTarget = this.getItemAtSlot(leftSlot);
            if (leftTarget && (!filterFn || filterFn(leftTarget))) {
                adjacentItems.push({
                    item: leftTarget,
                    position: 'left',
                    slot: leftSlot
        ...
        // 检查右侧相邻位置（考虑源物品的尺寸）
        const rightSlot = sourceSlot + sourceSize;
        if (rightSlot < this.inventory.length) {
            const rightTarget = this.getItemAtSlot(rightSlot);
            if (rightTarget && (!filterFn || filterFn(rightTarget))) {
                adjacentItems.push({
                    item: rightTarget,
                    position: 'right',
                    slot: rightSlot
    ...
    // 获取指定位置的实际物品（处理occupied情况）
    getItemAtSlot(slot) {
        if (slot < 0 || slot >= this.inventory.length) {
            return null;
        }
        
        const slotContent = this.inventory[slot];
        
        if (slotContent === null) {
            return null; // 空位置
        } else if (slotContent === 'occupied') {
            // 被占用的位置，查找实际物品
            return this.findItemByOccupiedSlot(slot);
        } else {
            // 直接的物品
            return slotContent;
        }
    ...
    // 药剂师特殊能力：召唤时加速相邻魔法物品1秒
    triggerAlchemistAcceleration(alchemistSlot) {
        const accelerationTime = 60; // 1秒 = 60帧
        
        console.log(`药剂师(位置${alchemistSlot})触发加速技能，开始检查相邻位置`);
        
        // 使用通用相邻查找函数，过滤魔法物品（包括召唤和非召唤的魔法物品）
        const adjacentMageItems = this.findAdjacentItems(
            alchemistSlot, 
            1, // 药剂师是小尺寸物品
            (item) => item.template.unitType === 'mage' // 移除unitCount限制，支持所有魔法物品
        );
        ...
        for (const adjacent of adjacentMageItems) {
            ...
            console.log(`✓ 药剂师为${position}侧魔法物品(${targetItem.template.name})提供1秒加速，加速时间: ${oldAcceleration} -> ${targetItem.accelerationTime}`);
        }
        
        if (acceleratedCount > 0) {
            console.log(`✓ 药剂师效果触发！${acceleratedCount}个相邻魔法物品获得1秒加速`);
            Utils.playSound('buff');
        } else {
            console.log('✗ 药剂师召唤，但没有相邻的魔法物品');
        }
    ...
    // 炼金工房特殊能力：产生金币
    triggerAlchemyLabGoldGeneration(item) {
        const goldAmount = item.alchemyGoldGeneration || 1;
        this.game.playerGold += goldAmount;
        
        console.log(`炼金工房产生了${goldAmount}金币！当前金币：${this.game.playerGold}`);
    ...
    getAlchemySellBonus() {
        ...
        for (let i = 0; i < this.inventory.length; i++) {
            const item = this.inventory[i];
            if (item && item !== 'occupied' && item.id === 'alchemyLab') {
                totalBonus += item.alchemySellBonus || 2;
                console.log(`战斗区炼金工房提供出售加成：+${item.alchemySellBonus || 2}`);
            }
        }
        ...
        for (let i = 0; i < this.backpack.length; i++) {
            ...
            if (item && item !== 'occupied' && item.id === 'alchemyLab') {
                totalBonus += item.alchemySellBonus || 2;
                console.log(`背包炼金工房提供出售加成：+${item.alchemySellBonus || 2}`);
            }
        }
    ...
    // 清空背包
    clearBackpack() {
        this.backpack = new Array(6).fill(null);
        console.log('背包已清空');
        this.updateBackpackDisplay();
    }
    
    // 向背包添加指定物品
    addSpecificItemToBackpack(itemId, quality = null) {
        const template = this.itemTemplates[itemId];
        if (!template) {
            console.log(`未找到物品模板: ${itemId}`);
            return false;
        }
        
        // 使用指定品质或默认最低品质
        const itemQuality = quality !== null ? quality : template.minQuality;
        ...
        const newItem = {
            ...
            staffMageBonus: template.id === 'staff' ? 10 : 0,
            staffWinBonus: 0,
            gladiatorBonus: 0,
            giantHealthBonus: 0,
            swordmasterBonus: 0
        ...
        if (newItem.id === 'giant') {
            const qualityLevelsAboveMin = newItem.quality - template.minQuality;
            newItem.giantHealthBonus = qualityLevelsAboveMin * 100;
        }
        ...
        if (success) {
            console.log(`成功添加${template.name}(品质${itemQuality})到背包`);
            this.updateBackpackDisplay();
            return true;
        } else {
